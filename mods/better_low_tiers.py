        ...
        if buff:
            for attack in fighter.attacks:
                for hb in attack.hitboxes:
                    hb.set_damage(hb.get_damage()+random.randint(1,2))
                    hb.set_growth(hb.get_growth()+random.randint(1,2)*10)
                    hb.set_base(hb.get_base()+random.randint(0,2)*5)
                    hb.set_size(hb.get_size()+random.randint(1,5)*100)
            fighter.set_weight(fighter.get_weight()+random.randint(1,4)*10)
            fighter.set_shield_size(fighter.get_shield_size()+random.randint(2,6))
            air_attributes = fighter.get_air_attributes()
            for attribute in air_attributes:
                attribute = attribute * random.uniform(1.1,1.25)
            fighter.set_air_attributes(air_attributes)
            ground_attributes = fighter.get_ground_attributes()
            ground_attributes[0] = ground_attributes[0] * random.uniform(1.1,1.25)
            ground_attributes[2] = ground_attributes[2] * random.uniform(1.1,1.25)
            ground_attributes[6] = ground_attributes[6] * random.uniform(1.1,1.25)
            ground_attributes[7] = ground_attributes[7] * random.uniform(1.1,1.25)
            ground_attributes[10] = ground_attributes[10] * random.uniform(1.1,1.25)
            ...
            for i in range(len(landing_attributes)-1):
                if i == 0:
                    ...
                    landing_attributes[i] = landing_attributes[i] - random.randint(5,12)
